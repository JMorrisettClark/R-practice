---
title: "Exploring Data Wrangling"
author: "Julie Clark"
date: "June 21, 2016"
output:
  word_document:
...
```
### 4. How many distinct names of each sex in each year?  (Use group_by)
...
Result <-
  Babynames %>%
  filter(name %in% c("Jane", "Mary")) %>% # just the Janes and Marys (filter because across rows)
  group_by(year, name) %>% # for each year for each name
  summarise(count = sum(n))  # add all male and female Janes and Marys because we did not filter by sex initially
```
...
ggplot(data=Result, aes(x = year, y = count)) +
  geom_line(aes(color=name),size=2)+ylab("Yearly Births")
```
...
```{r}
  ggplot(data=Result2, aes(x = year, y = proportion)) + geom_vline(xintercept = 1962)+
  geom_line(aes(color=name),size=2)+ylab("Yearly Proportion") 
```
...
Result <-
  Babynames %>%
  filter(name %in% c("Julie", "Janet", "Cathy","Laura")) %>% 
  group_by(year, name) %>% # for each year for each name
  summarise(count = sum(n))  
```
```{r}```{r}
ggplot(data=Result, aes(x = year, y = count)) +
  geom_line(aes(color=name),size=1)+ylab("Yearly Births")
```

```{r}
  Babynames %>%
  filter(name %in% c("Julie")) %>% 
  summarise(total = sum(n))
```
...
```
But how do we create a date object?
